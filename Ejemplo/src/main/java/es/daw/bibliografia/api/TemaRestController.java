...
public class TemaRestController {
	...
	@PutMapping("/api/temas/{id}")
	public ResponseEntity<Tema> editTema(@PathVariable long id, @RequestBody Tema tema) {
		Optional<Tema> tema2 = temaService.findOne(id);

		if (tema2.isPresent()) {
			tema2.get().setContenido(tema.getContenido());
			temaService.save(tema2.get());
			return new ResponseEntity<>(tema2.get(), HttpStatus.OK);
		} else {
